    ...
    if(success) {
      ...
      while (!it.IsEnd() && it->first == chunk.rowID) {
        ...
        if(aIndices->nodes[it->second] == myNode) {
          ...
          //logger->info() << "Multiplying (" <<  chunk.colID << " , " << aIndices->rowIDs[it->second] << ")" << logger->endl;
          ...
          //std::cout << "Joining " << "A : (" << aIndices->rowIDs[it->second] << ", " << aIndices->colIDs[it->second] << ") and (" << chunk.rowID << ", " << chunk.colID << ") => (" << processingChunk.rowID << ", " << processingChunk.colID << ")" << std::endl;
...
void aggregationProcessingStage(int32_t numNodes,
                                ...
                                std::atomic_int32_t *unfinishedJoinThreads,
  ...
  while(true) {
    ...
    // should we end this
    if (!success && (*unfinishedJoinThreads) == 0 && (*unfinishedReceiverThreads) == 0) {
      break;
    }
    ...
    if(success) {
      ...
      if(aggregateLocks->find(key) != aggregateLocks->end()) {
        ...
        // swap the elements
        auto tmp = aggregationChunk.block;
        aggregationChunk.block = sendingChunk.block;
        sendingChunk.block = tmp;

        // set the columns and rows
        sendingChunk.colID = aggregationChunk.colID;
        sendingChunk.rowID = aggregationChunk.rowID;
  ...
  std::cout << "Ended aggregationProcessingStage " << x << std::endl;
}
...
void aggregationSender(CommunicatorPtr communicator,
  ...
  while(true) {
    ...
    // should we end this
    if (!success && (*unfinishedJoinThreads) == 0) {
      break;
    }
...
void aggregationReceiver(CommunicatorPtr communicator,
  ...
  std::cout << "Ended aggregationReceiver" << std::endl;
...
//  for(int i = 0; i < aRowIDs.size(); ++i){
//    logger->info() << "A : " << aRowIDs[i] << ", " << aColIDs[i] << logger->endl;
//  }
//
//  for(int i = 0; i < bRowIDs.size(); ++i){
//    logger->info() << "B : " << bRowIDs[i] << ", " << bColIDs[i] << logger->endl;
//  }
  ...
  //logger->info() << aNodeOffset << " " << bNodeOffset;
  ...
  for(int i = 0; i < 2 * std::max(omp_get_max_threads(), communicator->getNumNodes()); ++i) {
    ...
    // create another
    MatrixChunk sendingProcessed{};
    sendingProcessed.block = new std::vector<double> (chunkSize * chunkSize);

    // insert the thing into the queue
    freeQueue.enqueue(chunk);
    freeProcessedQueue.enqueue(chunkProcessed);
    freeSendingQueue.enqueue(sendingProcessed);
  }
  ...
  std::atomic_int32_t unfinishedJoinThreads = resourceManager->getNumCores();
  ...
  for(int i = 0; i < resourceManager->getNumCores(); ++i) {
    ...
    auto *aggregationProcessingThread = new std::thread(aggregationProcessingStage,
                                                        ...
                                                        &freeSendingQueue,
                                                        &unfinishedJoinThreads,
                                                        &unfinishedReceiverThreads, logger);
  ...
  for(int i = 0; i < communicator->getNumNodes(); ++i) {
    ...
    auto *aggregationSenderThread = new std::thread(aggregationSender, communicator, &sendingQueue, &freeSendingQueue, &unfinishedJoinThreads);
